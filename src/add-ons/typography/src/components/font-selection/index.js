...
const { withSelect, withDispatch, useEffect } = wp.data;
...
const stylesElement = document.getElementById(
	"editorskit-typography-inline-css"
);
...
class TypographySelection extends Component {
	...
	setVariables(id) {
		...
		if (typeof fontData.content !== "undefined") {
			...
			if (content["font-family"]) {
				variables +=
					"--ek-font-family:" + wrapFontFamily(content["font-family"]) + ";";
		...
		if (typeof fontData.heading !== "undefined") {
			...
			if (heading["font-family"]) {
				variables +=
					"--ek-heading-font-family:" +
					wrapFontFamily(heading["font-family"]) +
					";";
		...
		map(contentCSS.heading, (headingData, headingType) => {
			...
			document.body.classList.remove(
				"ek-has-" + headingType + "-letter-spacing"
			);
			document.body.classList.remove("ek-apply-" + headingType + "-to-title");

			if (
				typeof fontData.heading !== "undefined" &&
				typeof fontData.heading[headingType] !== "undefined"
			) {
				...
				if (innerHeading["font-size"]) {
					variables +=
						...
						headingType +
						"-font-size:" +
						innerHeading["font-size"] +
						innerHeading["font-size-unit"] +
						";";
				...
				if (innerHeading["line-height"]) {
					variables +=
						...
						headingType +
						"-line-height:" +
						innerHeading["line-height"] +
						innerHeading["line-height-unit"] +
						";";
				...
				if (innerHeading["letter-spacing"]) {
					variables +=
						...
						headingType +
						"-letter-spacing:" +
						innerHeading["letter-spacing"] +
						innerHeading["letter-spacing-unit"] +
						";";
				...
				if (innerHeading["text-transform"]) {
					variables +=
						...
						headingType +
						"-text-transform:" +
						innerHeading["text-transform"] +
						";";
					document.body.classList.add(
						"ek-has-" + headingType + "-text-transform"
					);
				}
		...
		if (typeof fontData.button !== "undefined") {
			...
			if (button["font-family"]) {
				variables +=
					"--ek-button-font-family:" +
					wrapFontFamily(button["font-family"]) +
					";";
			...
			if (button["line-height"]) {
				variables +=
					"--ek-button-line-height:" +
					button["line-height"] +
					button["line-height-unit"] +
					";";
			...
			if (button["letter-spacing"]) {
				variables +=
					"--ek-button-letter-spacing:" +
					button["letter-spacing"] +
					button["letter-spacing-unit"] +
					";";
			...
			if (button["text-transform"]) {
				variables +=
					"--ek-button-text-transform:" + button["text-transform"] + ";";
	...
	createCSS(savedOption) {
		...
		if (contentCSS.fonts) {
			map(contentCSS.fonts, (fontProp) => {
				if (fontProp) {
					googleFontData +=
						fontProp.name.replace(" ", "+") + ":" + fontProp.weights + "|";
				}
			});
		}
	...
	getValue(category, key, level) {
		...
		if (level) {
			if (
				typeof contentCSS[category] !== "undefined" &&
				typeof contentCSS[category][level] !== "undefined" &&
				typeof contentCSS[category][level][key] !== "undefined"
			) {
				return contentCSS[category][level][key];
			}
		} else if (
			typeof contentCSS[category] !== "undefined" &&
			typeof contentCSS[category][key] !== "undefined"
		) {
	...
	onDelete() {
		...
		noticeMessage(
			__("Item successfully deleted.", "editorskit-typography-addon")
		);
	}
	...
	onSetDefault(isRemoving = false) {
		...
		noticeMessage(
			__("Default Typography Successfully Set.", "editorskit-typography-addon")
		);
	}

	render() {
		const { onFontSelection, removeFontSelection, postmeta, settingsPanel } =
			this.props;
		...
		if (!this.state.optionsFetched) {
			return (
				<Fragment>
					<p className="ek-fetching-typography">
		...
		if (typeof postmeta !== "undefined") {
			...
			if (
				typeof _editorskit_typography_data.meta !== "undefined" &&
				typeof _editorskit_typography_data.meta.id !== "undefined"
			) {
		...
		if (
			!currentMetaId &&
			!customKey &&
			typeof defaultTypography !== "undefined" &&
			defaultTypography &&
			typeof defaultTypography.itemSelected !== "undefined"
		) {
			...
			return (
				<Fragment>
					{!settingsPanel ? (
						...
						>
							<span>
								{__("Create Custom Combination", "editorskit-typography-addon")}
							</span>
						</Button>
					) : null}
					...
					<div className="editorskit-typography-selection">
						<ul className="editorskit-typography--items">
								...
								? map(optionsData, (fontCustom, id) => {
										if (
											...
											(fontCustom.name.toLowerCase().includes(searchKeyword) ||
												(typeof fontCustom.heading["font-family"] !==
													"undefined" &&
													fontCustom.heading["font-family"]
														.toLowerCase()
														.includes(searchKeyword)) ||
												(typeof fontCustom.content["font-family"] !==
													"undefined" &&
													fontCustom.content["font-family"]
														.toLowerCase()
														.includes(searchKeyword)) ||
												(typeof fontCustom.button["font-family"] !==
													"undefined" &&
													fontCustom.button["font-family"]
														.toLowerCase()
														.includes(searchKeyword)))
										) {
											//do nothing
										...
										return (
											<li
												className={classnames(
													...
													parseInt(id) === parseInt(currentMetaId) ||
														(!currentMetaId && customKey === id)
														? "editorskit-typography--item__current"
														: null
												)}
											>
												<Button
												...
												>
													...
													<span className="editorskit-typography--item__custom-fonts">
														{map(fontCustom, (fontCustomData) => {
															if (
																typeof fontCustomData["font-family"] !==
																"undefined"
															) {
																return (
																	<span>{fontCustomData["font-family"]}</span>
																);
															}
														})}
													</span>
												</Button>
												<TypographyDropdownMenu
													{...this.props}
													itemType="custom"
													selectedItem={id}
							...
							{map(Fonts, (font, i) => {
								if (
									searchKeyword &&
									!font.name.toLowerCase().includes(searchKeyword)
								) {
									return false;
								}
								return (
					...
					{isSettingDefault && (
						...
						>
							<p>
								{__(
									"Setting default typography will save you time when creating new posts or pages.",
									"editorskit-typography-addon"
								)}
							</p>
							<p className="editorskit-notice-box">
								{__(
									"Take note that this option will apply your selected default typography to old posts, post types and pages on your website without the need for you to edit them on the Gutenberg editor manually.",
									"editorskit-typography-addon"
								)}
							</p>
					...
					{isRemovingDefault && (
						...
						>
							<p>
								{__(
									"Are you sure you want to remove this item as default typography?",
									"editorskit-typography-addon"
								)}
							</p>
			...
			<Fragment>
				<div className="editorskit-typography-custom">
					...
					<TextControl
						...
						value={
							typeof contentCSS.name !== "undefined"
								? contentCSS.name
								: customKey
						}
						onChange={(newName) => {
					...
					>
						...
						/>
						{fontWeightOptions(this.getValue("content", "font-family")) &&
						fontWeightOptions(this.getValue("content", "font-family")).length >
							1 ? (
							<SelectControl
								label={__("Font Weight", "editorskit-typography-addon")}
								options={fontWeightOptions(
									this.getValue("content", "font-family")
								)}
						...
						<CheckboxControl
							label={__(
								"Apply Font Family to whole page",
								"editorskit-typography-addon"
							)}
							checked={this.getValue("content", "body") ? true : false}
							help={__(
								"Check this option to apply the selected font family to <body> instead of just content.",
								"editorskit-typography-addon"
							)}
					...
					>
						...
						>
							{(tab) => {
								...
								return (
									<Fragment>
										{fontWeightOptions(headingFontName) &&
										fontWeightOptions(headingFontName).length > 1 ? (
											<SelectControl
												label={sprintf(
										...
										{tab.level === "H1" ? (
											<CheckboxControl
												...
												checked={
													this.getValue("heading", "title", tab.level)
														? true
														: false
												}
												onChange={(selectedTransform) => {
					...
					>
						...
						/>
						{fontWeightOptions(this.getValue("button", "font-family")) &&
						fontWeightOptions(this.getValue("button", "font-family")).length >
							1 ? (
							<SelectControl
								label={__("Font Weight", "editorskit-typography-addon")}
								options={fontWeightOptions(
									this.getValue("button", "font-family")
								)}
