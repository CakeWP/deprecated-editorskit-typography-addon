...
function editorskitTypographySettings(FilteredComponent) {
	return (props) => {
		const [editorskitTypographyEnabled, setEditorskitTypographyEnabled] =
			useEntityProp("root", "site", "editorskit_typography_enabled");
		...
		useEffect(() => {
			const currentFeatureStatus = wp.data
